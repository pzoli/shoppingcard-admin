...
const Category = () => {
    ...
    const { data: categoryValues, status: dataFetchStatus, isLoading: isDataLoading } = useQuery({
        queryKey: ["category", lazyState],
        queryFn: async () => {
            const filters = encodeURIComponent(JSON.stringify(lazyState.filters));
            const res = await fetch(`/api/admin/crud/category?first=${lazyState.first}&rowcount=${lazyState.rows}&filter=${filters}`);
            return res.json();
        }
    });

    const { data: count, isLoading: isCountLoading } = useQuery<number>({
        queryKey: ["categorycount", lazyState],
        queryFn: async () => {
            const filters = encodeURIComponent(JSON.stringify(lazyState.filters));
            const res = await fetch(`/api/admin/crud/category/count?filter=${filters}`);
    ...
    const onSubmit = (data: any) => {
        ...
        if (editedRow && editedRow._id) {
            fetch('/api/admin/crud/category/' + editedRow._id, {
                method: 'PUT',
                credentials: "include",
                headers: {
    ...
    const deleteSelectedRow = () => {
        fetch('/api/admin/crud/category/' + selectedRow._id, {
            method: 'DELETE',
            credentials: "include",
            headers: {
                'Content-Type': 'application/json',
            ...
            show("success", `Deleted Category: ${JSON.stringify(data)}`);
            updatePage();
        }).catch((err) => show("error", err));
    }
    ...
    return (
        <div className="card">
            <h2>Category</h2>
            <Toast ref={toast} />
            <Dialog header="Category" visible={visible} onHide={() => setVisible(false)} style={{ width: '50vw' }}>
            ...
            <div className="card">
                <DataTable value={categoryValues}
